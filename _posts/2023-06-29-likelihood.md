...
$$P(吐两次金币｜中奖概率10 \%) = C_{10}^2 * (10 \%)^2 * (1 - 10 \%) ^ 8 \approx 19.37 \% $$


如果老虎机老虎机吐金币概率为$$20 \%$$，那么出现 玩家玩了十次，吐了两次金币的概率为：
$$P(吐两次金币｜中奖概率20 \%) = C_{10}^2 * (20 \%)^2 * (1 - 20 \%) ^ 8 \approx 30.20 \%$$


如果老虎机老虎机吐金币概率为$$90 \%$$，那么出现 玩家玩了十次，吐了两次金币的概率为：
$$P(吐两次金币｜中奖概率90 \%) = C_{10}^2 * (90 \%)^2 * (1 - 90 \%) ^ 8 \approx 0.000036 \%$$
...
这里令中奖概率为$$x$$，列出一下概率表达式：

$$P(吐两次金币｜中奖概率 x ) = C_{10}^2 * x^2 * (1 - x) ^ 8  = 45x^2(1-x)^8 $$

对上式求导：

$$P(吐两次金币｜中奖概率 x )' = 90x(1-x)^8 - 360x^2(1-x)^7 $$
